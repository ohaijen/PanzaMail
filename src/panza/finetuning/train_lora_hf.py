"""Hydra entrypoint for standard HF/PEFT LoRA fine-tuning."""
...
def tokenize_training_example(
    ...
    prompt, response = get_prompt_completion(example)
...
def get_model_dtype(cfg: DictConfig, device: torch.device) -> torch.dtype:
    if device.type == "mps":
        if cfg.model_precision == "bf16":
            warnings.warn(
                "bf16 precision is not supported on MPS; falling back to float16 for model weights."
            )
            return torch.float16
        if cfg.model_precision == "fp32":
            return torch.float32
        if cfg.model_precision == "4bit":
            raise ValueError("4bit loading is not supported on MPS for this trainer.")
...
def get_quantization_config(
    ...
    if weight_bias_dtype == "4bit":
        if device.type != "cuda":
            raise ValueError("4bit quantization is only supported on CUDA in this trainer.")
...
def main(cfg: DictConfig) -> None:
    ...
    model_dtype = get_model_dtype(cfg, device)
    quantization_config = get_quantization_config(cfg.finetuning.model, device)
    ...
    training_args = TrainingArguments(
        ...
        bf16=(cfg.finetuning.precision == "amp_bf16" and device.type == "cuda"),
        fp16=False,
